...
from src.data_prep import load_metadata, aggregate_diagnostic, generate_labels, butter_bandpass_filter, load_and_process_signals
...
def test_load_metadata_malformed_dict(mock_read_csv):
    ...
    mock_db = pd.DataFrame({
        'ecg_id': [1],
        'scp_codes': ["{'NORM': 100, malformed_part}"]
    }).set_index('ecg_id')
